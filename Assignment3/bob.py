...
class MySubscribeCallback(SubscribeCallback):
    ...
    def message(self, pubnub, message):
        ...
        cond = True
        fileexist = True
        #To check for the message sent by Alice after the mining for Bob is done. 
        #Loops to wait for Alice to send the message
        while(cond):
        	if(message.message !=""):
        		cond = False
        	else:
        		print("Waiting for Alice to mine")
        	time.sleep(3)
        #Retreiving the block number from the message
        blknum = message.message["Block number"]
        #Check if Bob created the json file for the current block chain
        while(fileexist):
        	...
        	if(check_file):
        		...
        		print("Waiting for Bob to finish mining")
        	time.sleep(3)
        #Retreiving the information from the file into a variable as a json
       	...
       	with open(str(blknum)+".json", "r") as f:
       		data = json.load(f)
        nonceofbob = data["Nonce"] - 1000000001
        nonceofalice = message.message["Nonce"]
        #If alice wins, it will overwrite the block chain json file created by bob
        if(nonceofalice<nonceofbob):
        	print("Alice has won this round")
...
for i in transactions:
	...
	pubnub.publish().channel('Channel-Alice').message({'Block number': blknum, 'Hash': prehash,'Transaction':i, 'Nonce':nonce}).pn_async(my_publish_callback)
	#A sleep timer to delay 
	time.sleep(10)
	message2 = input("Press the Enter key to mine next block once the result have been shown")
	#Once it reaches the end of the transaction, the program it exits
	if(i == 10):
